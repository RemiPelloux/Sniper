...
from src.ml.model import calculate_risk_scores, predict_vulnerabilities
...
def parse_arguments():
    ...
    parser.add_argument(
        "findings_file", help="Path to a JSON file containing security findings"
    )
    parser.add_argument(
        "--threshold",
        "-t",
...
def display_findings(findings, scores, threshold=0.5, top_n=10):
    ...
    sorted_findings = [(finding, scores.get(finding.id, 0)) for finding in findings]
    ...
    for i, (finding, risk_score) in enumerate(sorted_findings, 1):
        ...
        print(
            f"   Type: {Fore.CYAN}{getattr(finding, 'finding_type', 'unknown')}{Style.RESET_ALL}"
        )
        print(f"   Severity: {severity_color}{severity}{Style.RESET_ALL}")
        print(f"   Target: {Fore.BLUE}{finding.target}{Style.RESET_ALL}")
        print(
            f"   Risk Score: {get_probability_color(risk_score)}{risk_score:.2f}{Style.RESET_ALL}"
        )
    ...
    likely_vuln_count = sum(
        1 for f, _ in sorted_findings if probabilities.get(f.id, 0) >= threshold
    )

    print("\nSUMMARY:")
    print(f"Total findings analyzed: {len(findings)}")
    print(
        f"High risk findings (score >= 0.7): {Fore.RED}{high_risk_count}{Style.RESET_ALL}"
    )
    print(
        f"Likely vulnerabilities (probability >= {threshold}): {Fore.RED}{likely_vuln_count}{Style.RESET_ALL}"
    )
    print("=" * 80)
...
def main():
    ...
    try:
        ...
        if not findings:
            print(
                f"{Fore.RED}Error: No findings loaded from the file.{Style.RESET_ALL}"
            )
            return 1
