...
try:
    ...
    print(
        "Error: Sniper modules not found. Make sure you're running from the Sniper root directory."
    )
    sys.exit(1)
...
class ScanResultVisualizer:
    ...
    def load_from_master(
        self, task_id: str, master_host: str, master_port: int
    ) -> None:
    ...
    def generate_text_report(self) -> str:
        ...
        report.append(
            f"Execution Time: {self.data.get('execution_time', 0):.2f} seconds"
        )
    ...
    def generate_html_report(self) -> str:
        ...
        for vuln in self.data.get("vulnerabilities", []):
            severity_class = (
                f"vuln-{vuln.get('severity', '').lower()}"
                if vuln.get("severity", "").lower()
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize Sniper Security Tool scan results"
    )
    ...
    input_group.add_argument(
        "--task-id", type=str, help="Task ID to fetch from master node"
    )

    # Master node connection options (only needed with task-id)
    parser.add_argument(
        "--master-host", type=str, default="localhost", help="Master node hostname"
    )
    parser.add_argument(
