...
class StatusChecker:
    ...
    def check_integrations(self):
        ...
        for module, name in integration_modules:
            ...
            self.results["Integrations"].append((name, importable, error if not importable else "OK"))
    
    def check_plugins(self):
        """Check all plugins."""
        # Check sandbox plugin
        importable, error = self.check_module_importable("src.sniper.plugins.sandbox.sandbox_plugin")
        self.results["Plugins"].append(("Sandbox Plugin", importable, error if not importable else "OK"))
    ...
    def check_ml_modules(self):
        ...
        for module, name in ml_modules:
            importable, error = self.check_module_importable(module)
            self.results["ML Modules"].append((name, importable, error if not importable else "OK"))
    ...
    def check_core_components(self):
        ...
        for module, name in core_modules:
            importable, error = self.check_module_importable(module)
            self.results["Core Components"].append((name, importable, error if not importable else "OK"))
