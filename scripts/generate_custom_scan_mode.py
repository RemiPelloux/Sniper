...
def prompt_options(
    message: str, options: List[str], default: Optional[str] = None, allow_multiple: bool = False
) -> Union[str, List[str]]:
    """
...
def get_scan_mode_name() -> str:
    ...
    name = ""
    while not name:
        name = prompt_user("Enter a name for your custom scan mode (e.g., wordpress, microservices)")
        if not name:
...
def get_target_types() -> List[str]:
    ...
    target_types = [
        "domain", "url", "ip", "network", "webapp", "api", 
        "repository", "container", "cloud", "package"
    ]
...
def get_scan_modules() -> List[str]:
    ...
    message = "Select which modules to enable for this scan mode (comma-separated numbers):"
        ...
        if prompt_yes_no(f"Include {tool}?", True):
            ...
            if tool == "nmap":
                ...
                tools[tool]["options"]["ports"] = prompt_user(
                    "Ports to scan (e.g., 80,443 or top1000 or 1-65535)", 
                    "top1000"
                )
                
                timing_template = prompt_user("Timing template (1-5, higher is faster)", "3")
                try:
                ...
                severity = prompt_user(
                    "Severity levels (comma-separated)", 
                ...
                extensions = prompt_user("File extensions to scan (comma-separated)", "php,html,js")
...
def save_scan_mode(scan_mode: Dict, output_path: Optional[str] = None) -> Tuple[bool, str]:
    """
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", 
        help="Path to save the generated scan mode configuration"
    )
    parser.add_argument(
        "--update-main", "-u", action="store_true",
