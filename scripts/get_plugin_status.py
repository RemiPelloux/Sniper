...
class SniperStatusReport:
    ...
    def __init__(self):
        ...
        self.tools = {
            ...
            "zap": {
                "package": "zaproxy",
                "executable": ["zap.sh", "zap"],
                "docker_image": "owasp/zap2docker-stable",
            },
            "dirsearch": {
                "package": None,
                "executable": "dirsearch",
                "docker_image": "knqyf263/dirsearch",
            },
            "sublist3r": {
                "package": None,
                "executable": "sublist3r",
                "docker_image": "secsi/sublist3r",
            },
            "wappalyzer": {
    ...
    def check_package_installed(self, package_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"installed": True, "version": pkg.version, "location": pkg.location}
        except pkg_resources.DistributionNotFound:
            return {"installed": False}

    def check_executable_available(
        self, executable_name: Union[str, List[str]]
    ) -> Dict[str, Any]:
        ...
        if isinstance(executable_name, list):
            ...
            return {"available": True, "path": path} if path else {"available": False}
    ...
    def check_docker_image_available(self, image_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"available": False, "details": "Error checking for image"}
    ...
    def check_tool_status(self, tool_name: str) -> Dict[str, Any]:
        ...
        if tool_info.get("executable"):
            status["executable"] = self.check_executable_available(
                tool_info["executable"]
            )

        # Check Docker image
        if tool_info.get("docker_image"):
            status["docker"] = self.check_docker_image_available(
                tool_info["docker_image"]
            )
    ...
    def check_all_tools(self) -> None:
        ...
        for tool_name in self.tools:
            self.components["integrations"].append(
                {"name": tool_name, "status": self.check_tool_status(tool_name)}
            )
    ...
    def check_core_modules(self) -> None:
        ...
        for module in core_modules:
            try:
                ...
                self.components["core"].append({"name": module, "status": "OK"})
            except ImportError as e:
                self.components["core"].append(
                    {"name": module, "status": "FAILED", "error": str(e)}
                )
        ...
        try:
            ...
            for plugin_class in discovered:
                ...
                self.components["plugins"].append(
                    {"name": name, "status": "DISCOVERED"}
                )
        except ImportError as e:
            self.components["plugins"].append(
                {"name": "Plugin System", "status": "FAILED", "error": str(e)}
            )
    ...
    def check_ml_modules(self) -> None:
        ...
        for module in ml_modules:
            try:
                importlib.import_module(f"src.{module}")
                self.components["ml_modules"].append({"name": module, "status": "OK"})
            except ImportError as e:
    ...
    def check_dependencies(self) -> None:
        ...
        for dep, description in essential_deps.items():
            poetry_info = next(
                (p for p in self.poetry_metadata if p.get("name") == dep), None
            )
            ...
            self.components["dependencies"][dep] = {
                ...
                "version": (
                    pkg_info.get("version") if pkg_info.get("installed") else None
                ),
    ...
    def generate_report(self) -> None:
        ...
        for tool in self.components["integrations"]:
            ...
            if status.get("package"):
                ...
                if pkg_status.get("installed"):
                    self.console.print(
                        f"  Package: [green]Installed[/green] (v{pkg_status.get('version')})"
                    )
                else:
            ...
            if status.get("executable"):
                ...
                if exe_status.get("available"):
                    self.console.print(
                        f"  Executable: [green]Available[/green] ({exe_status.get('path')})"
                    )
                else:
            ...
            if status.get("docker"):
                ...
                if docker_status.get("available"):
                    self.console.print(
                        f"  Docker: [green]Available[/green] ({docker_status.get('details')})"
                    )
                else:
                    self.console.print(
                        f"  Docker: [yellow]Not available locally[/yellow]"
                    )

        # ML Modules
        self._print_module_table(
            "Machine Learning Modules", self.components["ml_modules"]
        )
        ...
        tools_ok = sum(
            1
            for t in self.components["integrations"]
            if t["status"]["package"].get("installed", False)
            or t["status"]["executable"].get("available", False)
            or t["status"]["docker"].get("available", False)
        )
        ...
        deps_ok = sum(
            1 for _, d in self.components["dependencies"].items() if d["installed"]
        )
        ...
        health_color = (
            ...
            else "yellow" if overall_health >= 0.6 else "red"
        )
        ...
        if overall_health < 1.0:
            ...
            if deps_ok < deps_total:
                self.console.print(
                    "• Install missing dependencies: [bold]poetry install[/bold]"
                )
            if tools_ok < tools_total:
                self.console.print(
                    "• Install missing security tools or ensure Docker is available"
                )
    ...
    def _print_module_table(self, title, modules):
        ...
        for module in modules:
            status_color = (
                "green" if module["status"] in ["OK", "DISCOVERED"] else "red"
            )
            table.add_row(
                module["name"], f"[{status_color}]{module['status']}[/{status_color}]"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--export", "-e", help="Export report to JSON file", metavar="FILE"
    )
