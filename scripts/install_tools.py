...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("tool_installation.log"), logging.StreamHandler()],
)
...
|____/|_| |_||_|| .__/  \___||_|   
 ...
 Security Tool Installer
    ...
    print(
        f"{Fore.YELLOW}This script will install security tools used by the Sniper Security Platform.{Style.RESET_ALL}"
    )
    print()
...
def get_install_choices(
    ...
    if categories:
        ...
        return {
            cat: tools for cat, tools in tools_by_category.items() if cat in categories
        }
    ...
    if not interactive:
        ...
        print_warning(
            "No tools or categories specified in non-interactive mode. No tools will be installed."
        )
        return {}
    ...
    for category, tools_list in tools_by_category.items():
        ...
        installed_count = sum(
            1 for tool in tools_list if installation_status.get(tool, False)
        )
        total_count = len(tools_list)

        print(
            f"{Fore.CYAN}=== {category.upper()} TOOLS ({installed_count}/{total_count} installed) ==={Style.RESET_ALL}"
        )
        for i, tool in enumerate(tools_list, 1):
            is_installed = installation_status.get(tool, False)
            status = (
                f"{Fore.GREEN}[Installed]"
                if is_installed
                else f"{Fore.RED}[Not Installed]"
            )
...
def install_tools(choices: Dict[str, List[str]], force: bool = False) -> None:
    ...
    already_installed = sum(
        1 for tool in all_tools_to_install if installation_status.get(tool, False)
    )
    to_install = len(all_tools_to_install) - already_installed

    print_info(
        f"Will install {to_install} new tools"
        + (f" and reinstall {already_installed} tools" if force else "")
    )
    print()
    ...
    for category, tools_list in sorted(choices.items()):
        ...
        print(
            f"{Fore.CYAN}=== Installing {category.upper()} tools ==={Style.RESET_ALL}"
        )
    ...
    if failed > 0:
        ...
        print_info(
            "You can try installing them individually or with a different installation method."
        )
...
def main() -> None:
    ...
    scope_group.add_argument(
        "--all", action="store_true", help="Install all available tools"
    )
    scope_group.add_argument(
        "--categories", nargs="+", help="Install tools in specific categories"
    )
    ...
    parser.add_argument(
        "--no-banner", action="store_true", help="Don't display the banner"
    )
