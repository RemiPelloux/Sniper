...
# Configuration
SOURCE_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)), "config", "tools.yaml")
...
def migrate_tools_config():
    ...
    logger.info(f"Migration complete: {success_count}/{len(tools)} tools successfully migrated")
...
if __name__ == "__main__":
    ...
    if count > 0:
        logger.info(f"Migration successful! {count} tools migrated to individual files.")
        sys.exit(0)
    else:
