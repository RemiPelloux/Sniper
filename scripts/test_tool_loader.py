...
def main():
    ...
    custom_tools_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "config", "custom_tools")
    ...
    for tool_name in custom_tools:
        ...
        if tool:
            logger.info(f"Custom tool '{tool_name}' was loaded successfully: {tool['description']}")
        else:
    ...
    if available_tools:
        ...
        logger.info(f"Installed tools: {', '.join(installed_tool_names[:5])}" + 
    ...
    for category in ToolCategory:
        ...
        if tool_names:
            logger.info(f"  Tools in {category.value}: {', '.join(tool_names[:5])}" + 
    ...
    for tool_name in sample_tools:
        ...
        if os_type == "darwin" and "brew" in install_info:
            ...
            logger.info(f"  Recommended cross-platform installation: {install_info['pip']}")
