    ...
    if args.provider_config:
        try:
            with open(args.provider_config, 'r') as f:
                ...
                logger.info(f"Loaded provider configuration from {args.provider_config}")
...
def start_worker(args):
    ...
    logger.info(f"Starting worker node connecting to {args.master_host}:{args.master_port}")
...
def main() -> int:
    ...
    master_parser.add_argument(
        ...
        choices=["round_robin", "capability_based", "priority_based", "load_balanced", "smart"],
    ...
    status_parser = subparsers.add_parser(
        "status", 
        help="Check status of distributed scanning deployment"
    )
