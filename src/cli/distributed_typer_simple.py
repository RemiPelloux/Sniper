...
def master_stop():
    ...
    if not _master:
        typer.echo(
            json.dumps(
                {"status": "error", "message": "No master node is running"}, indent=2
            )
        )
        return
...
def worker_start(
    ...
    worker_id: Optional[str] = typer.Option(
        None, help="Worker ID (auto-generated if not provided)"
    ),
    capabilities: str = typer.Option(
        "vuln_scan,recon", help="Comma-separated list of capabilities"
    ),
    ...
    _worker = MockWorker(
        master_host, master_port, worker_id, capabilities_list, max_tasks
    )
...
def worker_stop():
    ...
    if not _worker:
        typer.echo(
            json.dumps(
                {"status": "error", "message": "No worker node is running"}, indent=2
            )
        )
        return
...
def task_info(
    task_id: str = typer.Argument(..., help="ID of the task to get info about")
):
    ...
    if task:
        ...
        typer.echo(
            json.dumps(
                {"status": "error", "message": f"Task {task_id} not found"}, indent=2
            )
        )
...
def submit_task(
    task_type: str = typer.Option(
        ..., help="Type of task to submit (e.g., vulnerability_scan, recon)"
    ),
    ...
    try:
        ...
        typer.echo(
            json.dumps(
                {"status": "error", "message": "Invalid JSON in options"}, indent=2
            )
        )
        return
...
def submit_scan(
    ...
    scan_type: str = typer.Option(
        "full", help="Type of scan to perform (full, quick, vuln)"
    ),
