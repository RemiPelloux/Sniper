...
def predict(
    ...
    try:
        ...
        filtered_results = [
            (finding, score) for finding, score in prediction_results
            if score >= threshold
        ]
        
        logger.info(f"Found {len(filtered_results)} findings above threshold {threshold}")
...
def train(
    ...
    model_path: Path = typer.Option(
        "models/vulnerability_model.pkl", "--model-path", "-m", 
    ...
    try:
        ...
        if label_column not in df.columns:
            ...
            typer.echo(f"Error: Label column '{label_column}' not found in the data", err=True)
            raise typer.Exit(code=1)
        ...
        if len(findings) > 10:
            ...
            logger.info(f"Training on {len(train_findings)} samples, testing on {len(test_findings)} samples")
            typer.echo(f"Training on {len(train_findings)} samples, testing on {len(test_findings)} samples")
            ...
            typer.echo("Warning: Not enough data for train/test split, using all data for training")
...
def visualize(
    ...
    type: VisualizationType = typer.Option(
        VisualizationType.RISK_DISTRIBUTION, "--type", "-t", 
    ...
    try:
        ...
        if type == VisualizationType.RISK_DISTRIBUTION:
            ...
            severity_counts = Counter([finding.severity for finding in findings if finding.severity])
...
def output_risk_results(
    ...
    if format_type == OutputFormat.JSON:
        ...
        titles = [getattr(finding, 'title', f'Finding {finding.id}')[:30] for finding, _ in results]
        scores = [score for _, score in results]
        
        # Create chart
        plt.figure(figsize=(12, 8))
        colors = ['darkred' if score > 0.8 else 'red' if score > 0.6 
                  else 'orange' if score > 0.4 else 'yellow' 
                  for score in scores]
        ...
        for i, (finding, score) in enumerate(results, 1):
            lines.append(f"{i}. {getattr(finding, 'title', f'Finding {finding.id}')} (ID: {finding.id})")
            lines.append(f"   Type: {getattr(finding, 'finding_type', 'unknown')}")
            lines.append(f"   Severity: {getattr(finding, 'severity', 'unknown')}")
