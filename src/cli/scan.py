...
from src.integrations.base import ToolNotFoundError, BaseIntegration
...
def get_subdomain_tool(tool_name: str) -> Optional[Type[BaseIntegration]]:
    ...
    try:
        module = __import__(f"src.integrations.{tool_name}", fromlist=[tool_map[tool_name]])
    ...
    try:
        ...
        if scan_mode:
            ...
            if scan_mode_config:
                ...
                if "settings" in scan_mode_config and "scan_depth" in scan_mode_config["settings"]:
        ...
        unavailable_tools = [
            name for name, (available, _) in tool_availability.items()
            if not available
        ]
...
async def execute_scan(config: ScanConfig) -> Dict[str, List[Finding]]:
    ...
    for module in list(active_modules):
        ...
        if module == "subdomains":
            ...
            if not has_one_tool:
                console.print(f"Skipping {module} module: No required tools available", style="yellow")
            ...
            if missing_tools:
                console.print(f"Skipping {module} module: Missing required tools: {', '.join(missing_tools)}", style="yellow")
                active_modules.remove(module)
    ...
    for module in active_modules:
        try:
            ...
            if module_findings:
                findings[config.target] = findings.get(config.target, []) + module_findings
        ...
        for host in results.get("hosts", []):
            for port in host.get("ports", []):
                ...
                finding = Finding(
                    ...
                    severity=Severity.LOW if port["port"] in [80, 443] else Severity.MEDIUM,
                    confidence=90,
        ...
        zap = ZAPIntegration(
            ...
            options={
                **zap_config,
                ...
                "max_spider_duration": zap_config.get("max_spider_duration", max_spider_duration)
            }
        )
            ...
            if isinstance(zap_results, dict) and "alerts" in zap_results:
                for alert in zap_results["alerts"]:
                    finding = Finding(
                        ...
                        confidence=map_zap_confidence_to_int(alert.get('confidence', 'Low')),
                        target=target,
