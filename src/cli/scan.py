...
def configure_scan_parameters(
    ...
    log.info(
        f"Scan parameters: Depth={
            depth.name}, Threads={threads}, Timeout={timeout}s")
        ...
        if "," in module_str:
            for single_module in module_str.split(","):
                try:
                    ...
                    log.warning(
                        f"Invalid module string: {single_module}, skipping")
        else:
            try:
...
def scan(
    ...
    if scan_mode:
        ...
        scan_depth_str = mode_config.get(
            "settings", {}).get(
            "scan_depth", depth.name)
        scan_depth = str_to_scan_depth(scan_depth_str) if isinstance(
            scan_depth_str, str) else depth
        ...
        structured_reporting = mode_config.get(
            "settings", {}).get(
            "structured_reporting", False)
        prioritize_endpoints = mode_config.get(
            "settings", {}).get(
            "prioritize_endpoints", False)

        # Override modules with the ones from the scan mode
        if "modules" in mode_config:
            modules = [ScanModule(module) for module in mode_config["modules"]
                       if module in ScanModule._value2member_map_]
    ...
    unavailable_tools = [
        tool for tool,
        (available,
         _) in tool_availability.items() if not available]
    if unavailable_tools:
        typer.echo(
            f"Warning: Some tools are not available: {
    ...
    with Progress(
        ...
        try:
            # Run each selected module with appropriate tools and
            # configurations
            if ScanModule.TECHNOLOGIES.value in module_list:
                if tool_availability.get("wappalyzer", (False, ""))[0]:
                    tech_tools = {}
                    if scan_mode:
                        # Get tool configurations for this module from the scan
                        # mode
                        mode_tools = scan_mode_manager.get_tools_for_scan_mode(
                            scan_mode)
                    ...
                    if scan_mode:
                        ...
                        for tool in ["sublist3r", "amass", "subfinder"]:
                            if (
                                ...
                                subdomain_tools[tool] = mode_tools[tool].get(
                                    "options", {})
            ...
            if ScanModule.PORTS.value in module_list:
                if tool_availability.get("nmap", (False, ""))[0]:
                    ...
                    if scan_mode:
                        ...
                        mode_tools = scan_mode_manager.get_tools_for_scan_mode(
                            scan_mode)
                        if "nmap" in mode_tools and mode_tools["nmap"].get(
                                "enabled", True):
                            port_tools["nmap"] = mode_tools["nmap"].get(
                                "options", {})
            ...
            if ScanModule.WEB.value in module_list:
                if tool_availability.get("zap", (False, ""))[0]:
                    ...
                    if scan_mode:
                        ...
                        mode_tools = scan_mode_manager.get_tools_for_scan_mode(
                            scan_mode)
                        if "zap" in mode_tools and mode_tools["zap"].get(
                                "enabled", True):
                            web_tools["zap"] = mode_tools["zap"].get(
                                "options", {})
            ...
            if ScanModule.DIRECTORIES.value in module_list:
                if tool_availability.get("dirsearch", (False, ""))[0]:
                    ...
                    directory_findings = asyncio.run(run_directory_scan(
                        target, scan_depth, ignore_ssl, dir_tools))
                    all_findings.extend(directory_findings)
                else:
                    log.warning(
                        "Skipping directories module: Dirsearch not available")
            ...
            # If using AI Smart scan mode with prioritization and structured
            # reporting
            if scan_mode == "ai_smart" and prioritize_endpoints:
                try:
                    ...
                    for tool_name, result in scan_results.items():
                        if isinstance(
                                result, dict) and "crawled_urls" in result:
                    ...
                    if crawled_urls:
                        ...
                        console.print(
                            f"Prioritizing {
                                len(crawled_urls)} discovered URLs...",
                            style="blue")
                        prioritized_urls = prioritizer.prioritize(
                            crawled_urls, target)

                        # Create structured report
                        console.print(
                            "Generating structured report...", style="blue")
                    ...
                    console.print(
                        f"Warning: Could not import URL prioritizer: {
                            str(e)}", style="yellow")
                    console.print(
...
def resolve_scan_modules(modules: List[ScanModule]) -> List[str]:
    ...
    if isinstance(
            modules,
            list) and len(modules) > 0 and isinstance(
            modules[0],
            str):
...
async def run_directory_scan(
    ...
    log.debug(
        f"Running directory scan against {target} with depth {
            depth.value}")
...
def output_scan_results(
    ...
    if correlated_findings:
        ...
        total_findings = sum(len(findings)
                             for findings in correlated_findings.values())
...
def validate_target_url(target: str) -> str:
    ...
    if not target.startswith(("http://", "https://")):
        ...
        if all(part.isdigit() and 0 <= int(part) <=
               255 for part in target.split(".") if part):
...
def scan_juiceshop(
        target: str = typer.Argument(
            "http://localhost:3000",
            help="JuiceShop URL (default: http://localhost:3000)"),
    output: Optional[Path] = typer.Option(
            None,
            "--output",
            "-o",
            help="Output file for detailed findings"),
        json_format: bool = typer.Option(
            False,
            "--json",
            "-j",
        ...
        wait_time: int = typer.Option(
            3,
            "--wait",
            help="Wait time in seconds for JavaScript to load"),
) -> None:
    """
    ...
    console.print(
        f"[bold green]Starting specialized Juice Shop scan against:[/] {target}")
    console.print(
        "[bold yellow]This scan will test for: XSS, SQLi, Open Redirect, Path Traversal[/]")
    console.print(
        f"[bold blue]Crawl settings:[/] Max URLs: {max_urls}, Wait time: {wait_time}s")
    ...
    with Progress(
        ...
        if not scanner.check_prerequisites():
            console.print(
                "[bold red]Error:[/] Vulnerability scanner prerequisites not met.")
            raise typer.Exit(code=1)
        ...
        try:
            ...
            if findings:
                console.print(
                    f"\n[bold green]Found {
                        len(findings)} potential vulnerabilities:[/]")
                ...
                for finding in findings:
                    ...
                    if not evidence and hasattr(finding, "raw_evidence"):
                        evidence = str(finding.raw_evidence)[:80] + "..." if finding.raw_evidence and len(
                            str(finding.raw_evidence)) > 80 else str(finding.raw_evidence or "")
                    ...
                    with open(output, "w") as f:
                        if json_format:
                            ...
                            for i, finding in enumerate(findings, 1):
                                ...
                                f.write(
                                    f"- Request Method: {finding.request_method}\n")
                                f.write(
                                    f"- Evidence:\n```\n{finding.evidence}\n```\n\n")
                ...
                console.print(
                    "1. Try SQL injection on the login form (admin' --)")
                console.print(
                    "2. Try XSS in the search field (<script>alert('xss')</script>)")
                console.print(
                    "3. Check for path traversal in the FTP server (%2e%2e/%2e%2e/etc/passwd)")
                console.print(
                    "4. Try to access /score-board directly for vulnerability tracking")
...
def scan_dvwa(
        target: str = typer.Argument(
            "http://localhost:80",
            help="DVWA URL (default: http://localhost:80)"),
    output: Optional[Path] = typer.Option(
            None,
            "--output",
            "-o",
        ...
        login: bool = typer.Option(
            True,
            "--login/--no-login",
            help="Automatically login to DVWA before scanning"),
        security_level: str = typer.Option(
            "low",
            "--security-level",
            help="DVWA security level to set before scanning (low, medium, high, impossible)"),
) -> None:
    """
    ...
    console.print(
        f"[bold green]Starting specialized DVWA scan against:[/] {target}")
    console.print(
        "[bold yellow]This scan will test for: XSS, SQLi, Command Injection, Path Traversal, File Inclusion[/]")
    console.print(
        f"[bold blue]Crawl settings:[/] Max URLs: {max_urls}, Wait time: {wait_time}s")

    if login:
        console.print(
            f"[bold blue]Login:[/] Will attempt to login with default credentials")
        console.print(
            f"[bold blue]Security Level:[/] Will set to {security_level}")
    ...
    with Progress(
        ...
        try:
            ...
            if login:
                ...
                login_successful = try_dvwa_login(
                    scanner, target, security_level)

                if not login_successful:
                    console.print(
                        "[bold red]Warning:[/] Failed to login to DVWA. Continuing with unauthenticated scan.")
            ...
            if findings:
                ...
                console.print(
                    "1. Try SQL injection on the login form (admin' --)")
                console.print(
                    "2. Try XSS in the user input fields (<script>alert('xss')</script>)")
                console.print(
                    "3. Try command injection in the ping tool (127.0.0.1 && cat /etc/passwd)")
                console.print(
                    "4. Try path traversal and file inclusion (../../etc/passwd)")
...
def try_dvwa_login(scanner, target, security_level):
    ...
    try:
        ...
        if response.status_code != 200:
            log.warning(
                f"Failed to access DVWA login page: {
        ...
        csrf_token = None
        match = re.search(
            r'<input type="hidden" name="user_token" value="([a-zA-Z0-9]+)"',
            response.text)
        if match:
        ...
        login_response = scanner.session.post(
            login_url, data=login_data, timeout=10, allow_redirects=True)

        # Check if login was successful (look for logout link or welcome
        # message)
        if "logout" in login_response.text.lower(
        ) or "welcome to damn vulnerable web application" in login_response.text.lower():
            ...
            if security_level in ["low", "medium", "high", "impossible"]:
                ...
                security_response = scanner.session.get(
                    security_url, timeout=10)
                ...
                security_set_response = scanner.session.post(
                    security_url, data=security_data, timeout=10)

                if "security level set to" in security_set_response.text.lower():
                    log.info(
                        f"Successfully set DVWA security level to {security_level}")
                else:
                    log.warning(
                        f"Failed to set DVWA security level to {security_level}")
