...
# FastAPI app
app = FastAPI(title="Sniper Task Submitter", description="Submit tasks to Sniper's distributed scanning system")
...
class TaskSubmission(BaseModel):
    ...
    task_type: str = Field(..., description="Type of task (e.g., vulnerability_scan, recon)")
    priority: str = Field("medium", description="Task priority (low, medium, high)")
    options: Dict[str, Any] = Field(default_factory=dict, description="Additional task options")
...
class VulnScanRequest(BaseModel):
    target_url: str = Field(..., description="Target URL to scan")
    vulnerability_type: Optional[str] = Field(None, description="Specific vulnerability type to test")
...
class ReconRequest(BaseModel):
    ...
    techniques: List[str] = Field(default_factory=list, description="Specific recon techniques to use")
    priority: str = Field("medium", description="Task priority (low, medium, high)")
...
async def submit_task_async(
    ...
    if task_type == "vulnerability_scan":
        ...
        if vuln_type:
            logger.info(f"Submitting vulnerability scan for {target_url} with type {vuln_type}")
        ...
        logger.warning(f"Task type '{task_type}' not directly supported, using generic submission")
        return {
            "status": "error",
...
async def health():
    ...
    try:
        ...
        return {
            "status": "unhealthy",
            "message": "Not connected to master node"
        }
    ...
    try:
        if scan.vulnerability_type:
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Invalid vulnerability type: {scan.vulnerability_type}")
        ...
        if task_id:
            ...
            raise HTTPException(status_code=500, detail="Failed to submit task to master node")
...
async def connect_to_master(host: str, port: int, retries: int = 5, retry_delay: int = 5):
    ...
    for attempt in range(retries):
        try:
            ...
            logger.warning(f"Failed to connect to master node (attempt {attempt+1}/{retries})")
        except Exception as e:
...
def main():
    ...
    parser.add_argument("--master", default="localhost:5000", help="Master node address (host:port)")
    ...
    try:
        ...
        logger.error(f"Invalid master address format: {args.master}. Use host:port format.")
        sys.exit(1)
