...
class AutoScaler:
    ...
    def _collect_metrics(self) -> Dict[str, float]:
        ...
        active_workers = len(
            [
                w
                for w in self.master_node.workers.values()
    ...
    def _scale_up(self):
        ...
        try:
            ...
            threading.Thread(
                target=self._wait_for_node_registration, args=(worker_id,), daemon=True
            ).start()
            ...
            with self.master_node.worker_lock:
                for node_id, node_info in self.master_node.workers.items():
                    if node_id == worker_id or (
                        hasattr(node_info, "worker_id")
                        and node_info.worker_id == worker_id
                    ):
...
class DockerWorkerProvider(WorkerProviderBase):
    ...
    def remove_worker(self, worker_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error removing Docker container for worker {worker_id}: {e}",
                exc_info=True,
            )
            return False
...
class KubernetesWorkerProvider(WorkerProviderBase):
    ...
    def create_worker(self, master_host: str, master_port: int) -> Optional[str]:
        ...
        try:
            ...
            pod_manifest = {
                ...
                "metadata": {
                    "name": pod_name,
                    "labels": {"app": "sniper-worker", "worker-id": worker_id},
                },
                "spec": {
                    "containers": [
                        {
            ...
            if "resources" in self.config:
                pod_manifest["spec"]["containers"][0]["resources"] = self.config[
                    "resources"
                ]
    ...
    def remove_worker(self, worker_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error deleting Kubernetes pod for worker {worker_id}: {e}",
                exc_info=True,
            )
            return False
