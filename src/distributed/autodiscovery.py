    ...
    def start(self) -> bool:
        ...
        try:
            ...
            if self.config["discovery"]["enabled"]:
                ...
                logger.info(f"Discovery manager started with method: {self.discovery_method}")
    ...
    def _start_local_worker(self, worker_id: str) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                "worker",
                "--master-host", 
                "localhost" if self.config["master"]["host"] == "0.0.0.0" else self.config["master"]["host"],
                "--master-port", 
            ...
            logger.error(f"Failed to start local worker {worker_id}: {str(e)}", exc_info=True)
            return False
