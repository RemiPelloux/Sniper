...
from src.distributed.base import NodeInfo, NodeStatus, NodeRole
...
def create_master_app(master_node) -> FastAPI:
    ...
    """
    app = FastAPI(title="Sniper Master Node API", 
    ...
    @app.get("/get-task/{worker_id}", response_model=Union[GenericResponse, Dict[str, Any]])
    def get_task(worker_id: str):
        ...
        try:
            ...
            if task:
                ...
                return GenericResponse(
                    success=True,
                    message="No tasks available for worker",
                    data=None
                )
        except Exception as e:
...
def create_worker_app(worker_node) -> FastAPI:
    ...
    Returns:
        Configured FastAPI application
    """
    app = FastAPI(title="Sniper Worker Node API",
    ...
    def receive_task(task_data: Dict[str, Any]):
        ...
        try:
            ...
            Thread(target=worker_node.handle_task, args=(task_data,), daemon=True).start()
