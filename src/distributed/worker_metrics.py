...
class WorkerMetrics:
    ...
    def update_response_time(self, response_time: float):
        ...
        if self.response_time == 0:
            ...
            self.response_time = (self.response_time * (1 - alpha)) + (response_time * alpha)
            
    def get_average_execution_time(self, task_type: str) -> float:
        """Get average execution time for a task type"""
        if task_type in self.task_execution_times and self.task_execution_times[task_type]:
    ...
    def get_health_score(self) -> float:
        ...
        if self.last_heartbeat:
            seconds_since_heartbeat = (datetime.now(timezone.utc) - self.last_heartbeat).total_seconds()
...
class WorkerMetricsManager:
    ...
    def update_task_result(self, worker_id: str, task_type: str, 
    ...
    def get_healthy_workers(self) -> Dict[str, WorkerMetrics]:
        ...
        return {wid: metrics for wid, metrics in self.metrics.items() 
                if metrics.is_healthy()}
                
    def get_available_workers_for_task(self, task_type: str) -> Dict[str, WorkerMetrics]:
    ...
    def get_best_worker_for_task(self, task_type: str) -> Optional[str]:
        ...
        predictions = {wid: metrics.predicted_completion_time(task_type) 
    ...
    def get_system_statistics(self) -> Dict[str, float]:
        ...
        total_capacity = sum(m.max_concurrent_tasks for m in self.metrics.values() if m.is_healthy())
        ...
        if active_metrics:
            avg_success_rate = sum(m.success_rate for m in active_metrics) / len(active_metrics)
            avg_load = sum(m.current_load for m in active_metrics) / len(active_metrics)
            avg_response_time = sum(m.response_time for m in active_metrics) / len(active_metrics)
            system_health = sum(m.get_health_score() for m in active_metrics) / len(active_metrics)
        else:
            avg_success_rate = 0
            avg_load = 0
            avg_response_time = 0
