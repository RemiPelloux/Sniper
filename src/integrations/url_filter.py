...
class UrlFilter:
    ...
    def __init__(
        ...
        self.compiled_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.excluded_patterns
        ]
    ...
    def is_allowed(self, url: str, source_url: str = None) -> bool:
        ...
        if self.include_only_same_domain:
            ...
            if self.base_domain and url_domain != self.base_domain:
                ...
                if self.include_subdomains:
                    if (
                        not url_domain.endswith(f".{self.base_domain}")
                        and url_domain != self.base_domain
                    ):
                        return False
                else:
                    return False
        ...
        if parsed_url.fragment:
            ...
            if source_url:
                ...
                if (
                    parsed_url.netloc == source_parsed.netloc
                    and parsed_url.path == source_parsed.path
                    and parsed_url.params == source_parsed.params
                    and parsed_url.query == source_parsed.query
                ):
                    return False
