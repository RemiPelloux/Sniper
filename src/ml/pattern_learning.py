...
class PatternLearner:
    ...
    def __init__(self, model_dir: str = None, use_deep_learning: bool = False):
        ...
        self.security_terms = {
            "xss", "csrf", "sqli", "injection", "overflow", "dos", "ddos", 
            "authentication", "authorization", "privilege", "escalation",
            "bypass", "mitm", "clickjacking", "hijacking"
        }
        ...
        if self.deep_learning:
            try:
                ...
                logger.warning("TensorFlow not available, falling back to traditional ML")
    ...
    def _preprocess_text(self, text: str) -> str:
        ...
        tokens = [self.lemmatizer.lemmatize(token) for token in tokens if token not in self.custom_stopwords]
    ...
    def _cluster_findings(self, feature_vectors: np.ndarray, 
                          min_samples: int = 2, eps: float = 0.5) -> np.ndarray:
        """
    ...
    def _analyze_clusters(self, clusters: Dict[str, List[Dict]]) -> Dict[str, Any]:
        ...
        for label, items in clusters.items():
            ...
            cluster_stats[label] = {
                "count": count,
                "main_finding_type": finding_types.most_common(1)[0][0] if finding_types else None,
                "main_severity": severities.most_common(1)[0][0] if severities else None,
    ...
    def find_similar_patterns(self, finding: Dict, max_results: int = 5) -> List[Dict]:
        ...
        try:
            ...
            # Vectorize text
            query_vector = self.tfidf_vectorizer.transform([features["preprocessed_text"]])
            ...
            similarity_results = [
                {
                    ...
                    "cluster": str(cluster_label) if cluster_label is not None else "unknown"
                }
    ...
    def extract_common_patterns(self, findings: List[Dict], min_pattern_support: float = 0.1) -> List[Dict]:
        """
        ...
        try:
            ...
            common_ngrams = {ngram: count for ngram, count in ngram_counter.items() 
                            if count >= min_count and len(ngram.split()) > 1}  # Only multi-word patterns
