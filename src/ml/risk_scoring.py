...
class RiskScorer:
    ...
    def _extract_features(self, finding: Dict) -> np.ndarray:
        ...
        has_poc = 1.0 if "POC" in description or "proof of concept" in description.lower() else 0.0
        ...
        has_url = 1.0 if ("http://" in description or "https://" in description) else 0.0
    ...
    def prioritize_findings(self, findings: List[Dict], env_context: Dict = None) -> List[Dict]:
        """
    ...
    def train_risk_model(self, findings: List[Dict], risk_scores: List[float]) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully trained risk model with {len(findings)} examples")
            return True
    ...
    def analyze_risk_trends(self, findings_history: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            first_quarter_avg = sum(scores[:quarter_size]) / quarter_size if scores else 0
            last_quarter_avg = sum(scores[-quarter_size:]) / quarter_size if scores else 0
            
            # Trend direction
            trend_direction = "improving" if last_quarter_avg < first_quarter_avg else "worsening" if last_quarter_avg > first_quarter_avg else "stable"
            ...
            for severity, data in severities.items():
                if data:
                    ...
                    if len(data) >= 2:
                        ...
                        second_half_avg = sum(s for _, s in second_half) / len(second_half)
                        
                        direction = "improving" if second_half_avg < first_half_avg else "worsening" if second_half_avg > first_half_avg else "stable"
                        severity_trends[severity] = {
                            "direction": direction,
                            "change_rate": (second_half_avg - first_half_avg) / (time_span / 30)  # Normalized to change per month
                        }
            ...
            return {
                ...
                "overall_trend": {
                    ...
                    "change_rate": (last_quarter_avg - first_quarter_avg) / (time_span / 30)  # Normalized to change per month
                },
    ...
    def adjust_cvss_score(self, cvss_base: float, temporal_factors: Dict = None, env_factors: Dict = None) -> float:
        """
        ...
        if temporal_factors:
            ...
            remediation_level = temporal_factors.get("remediation_level", 1.0)  # 0.9-1.0
            report_confidence = temporal_factors.get("report_confidence", 1.0)  # 0.9-1.0
            
            # Adjust temporal score (simplified CVSS formula)
            temporal_score = cvss_base * exploit_code * remediation_level * report_confidence
        
        # Apply environmental factors if provided
        env_score = temporal_score
        if env_factors:
            # Example environmental factors (simplified)
            confidentiality_req = env_factors.get("confidentiality_requirement", 1.0)  # 0.5-1.5
            integrity_req = env_factors.get("integrity_requirement", 1.0)  # 0.5-1.5
            availability_req = env_factors.get("availability_requirement", 1.0)  # 0.5-1.5
    ...
    def calculate_integrated_risk(self, findings: List[Dict], asset_value: float = 5.0, 
                                  threat_level: float = 5.0) -> Dict[str, Any]:
        """
        ...
        integrated_risk = (0.4 * max_risk) + (0.3 * weighted_severity) + (0.2 * asset_value_norm) + (0.1 * threat_level_norm)
