...
class ToolSelector:
    ...
    def __init__(self, model_dir: str = None, performance_history_size: int = 1000):
        ...
        self.all_tools = [tool for tools in self.tool_categories.values() for tool in tools]
    ...
    def _load_models(self):
        ...
        for category in self.tool_categories:
            model_path = os.path.join(self.model_dir, f"{category}_tool_selector.joblib")
        ...
        for category, model in self.models.items():
            if model is not None:
                ...
                try:
                    ...
                    logger.info(f"Saved tool selection model for {category} to {model_path}")
                except Exception as e:
            ...
            try:
                with open(history_path, 'r') as f:
                    ...
                    for tool, records in history_data.items():
                        self.performance_history[tool] = records[-self.performance_history_size:]
    ...
    def _extract_target_features(self, target_info: Dict) -> Dict:
        ...
        features["is_web_application"] = int("web" in features["target_type"] or 
                                           target_info.get("has_web_interface", False))
        features["is_api"] = int("api" in features["target_type"] or 
                               target_info.get("has_api", False))
        features["is_mobile"] = int("mobile" in features["target_type"])
        features["is_network"] = int("network" in features["target_type"] or 
                                   target_info.get("has_network_services", False))
        ...
        features["has_http_ports"] = int(80 in target_info.get("open_ports", []) or 
                                      443 in target_info.get("open_ports", []))
        features["has_db_ports"] = int(any(port in target_info.get("open_ports", []) 
        ...
        features["time_constraint"] = target_info.get("time_constraint", 0)  # In minutes, 0 means no constraint
            ...
            for entry in training_data:
                ...
                for category, tools in self.tool_categories.items():
                    ...
                    if best_tool and best_score > 0:
                        ...
                        if len(self.performance_history[best_tool]) > self.performance_history_size:
                            self.performance_history[best_tool] = self.performance_history[best_tool][-self.performance_history_size:]
    ...
    def record_tool_performance(self, tool: str, target_info: Dict, 
        ...
        try:
            ...
            if "finding_count" in performance_metrics:
                ...
                finding_score = min(1.0, finding_count / 10)  # Assume 10+ findings is good
                effectiveness = max(effectiveness, finding_score)
            
            if "true_positives" in performance_metrics and "false_positives" in performance_metrics:
                # Calculate precision
            ...
            if "execution_time" in performance_metrics and "time_limit" in performance_metrics:
            ...
            if len(self.performance_history[tool]) > self.performance_history_size:
                self.performance_history[tool] = self.performance_history[tool][-self.performance_history_size:]
            
            # Save updated history
            self._save_performance_history()
            
            logger.info(f"Recorded performance for {tool}: effectiveness={effectiveness:.2f}")
            
        except Exception as e:
            logger.error(f"Error recording tool performance: {e}")
    
    def recommend_tools(self, target_info: Dict, max_tools: int = 3) -> Dict[str, List[Dict]]:
        """
            ...
            for category, tools in self.tool_categories.items():
                ...
                for tool in tools:
                    ...
                    if features["time_constraint"] > 0:
                        if tool in ["nmap", "owasp_zap"] and features["time_constraint"] < 30:
                            score -= 0.1
    ...
    def plan_scan_sequence(self, target_info: Dict, selected_tools: List[str]) -> List[Dict]:
        """
        ...
        try:
            ...
            tool_dependencies = {
                ...
                "nikto": ["nmap"],
                "sqlmap": ["owasp_zap"],  # Better after finding potential injection points
                "dirsearch": ["nmap"],
                "gobuster": ["nmap"],
                "ffuf": ["nmap"],
            ...
            for phase, phase_tools in tool_phases.items():
                ...
                for tool in phase_tools:
                    if tool in remaining_tools:
                        ...
                        deps = [dep for dep in tool_dependencies.get(tool, []) if dep in available_tools]
                ...
                for tool in phase_tools_to_run:
                    ...
                    if tool in tool_dependencies and tool_dependencies[tool]:
                        deps = [dep for dep in tool_dependencies[tool] if dep in available_tools]
                        if deps:
