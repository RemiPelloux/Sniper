...
def load_payloads(category: str, payload_file: str = "default.json") -> List[str]:
    ...
    if not payload_path.exists():
        raise FileNotFoundError(
            f"Payload file '{payload_file}' not found in category '{category}'"
        )
...
def add_payload(
    category: str, payload: str, payload_file: str = "default.json"
) -> bool:
    """
...
def mutate_payloads(
    ...
    try:
        ...
        raise ImportError(
            "Mutation engine is not available. Ensure src/payloads/mutation_engine.py and src/payloads/mutator.py exist."
        )

    mutator = PayloadMutator(complexity=complexity)
    return mutator.mutate_payloads(
        payloads, vulnerability_type, context, num_variations
    )
