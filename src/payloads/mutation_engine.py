...
class MutationEngine:
    ...
    def __init__(self, complexity: int = 3):
        ...
        logger.info(
            f"Initialized MutationEngine with complexity level {self.complexity}"
        )
    ...
    def mutate(
        ...
        if vuln_type in self.vuln_type_mutators:
            ...
            variations = self.vuln_type_mutators[vuln_type](
                payload, context, num_variations
            )
        else:
    ...
    def _mutate_generic(self, payload: str, num_variations: int) -> List[str]:
        ...
        for _ in range(target_count):
            ...
            if (
                len(variations) >= num_variations + 1
            ):  # +1 because we include the original
                break
    ...
    def _mutate_xss(
        self, payload: str, context: Optional[str], num_variations: int
    ) -> List[str]:
