...
class PayloadMutator:
    ...
    def mutate_payload(
        ...
        """
        logger.debug(
            f"Generating {num_variations} mutations for {vulnerability_type} payload: {payload}"
        )
    ...
    def mutate_payloads(
        ...
        vulnerability_type: str,
        context: Optional[str] = None,
        num_variations_per_payload: int = 2,
    ) -> List[str]:
        """
        ...
        for payload in payloads:
            variations = self.mutate_payload(
                payload, vulnerability_type, context, num_variations_per_payload
            )
            result.extend(variations)

        logger.debug(
            f"Generated {len(result) - len(payloads)} variations from {len(payloads)} payloads"
        )
        return result

    def enhance_payloads(
        self, payloads: List[Dict[str, Any]], num_variations: int = 2
    ) -> List[Dict[str, Any]]:
        """
        ...
        for payload in payloads:
            ...
            variations = self.mutate_payload(
                payload["value"], payload["type"], context, num_variations
            )
            ...
            for idx, var_value in enumerate(variations):
                ...
                var_payload["description"] = (
                    f"Variation {idx+1} of: {payload.get('description', 'payload')}"
                )
        ...
        logger.debug(
            f"Enhanced payload set from {len(payloads)} to {len(result)} items"
        )
        return result
