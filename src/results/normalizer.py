...
class ResultNormalizer:
        ...
        if tool_name:
            ...
            for tool, tool_findings in tool_groups.items():
                ...
                if normalizer:
                    ...
                    normalized_findings.extend(default_normalizer.normalize(tool_findings))
    ...
    def deduplicate_findings(self, findings: List[BaseFinding]) -> List[BaseFinding]:
        ...
        tech_findings = [f for f in findings_by_type.get(TechnologyFinding, [])
        ...
        for finding_type, findings_list in findings_by_type.items():
            if finding_type not in {PortFinding, WebFinding, SubdomainFinding, TechnologyFinding}:
                deduplicated.extend(findings_list)
                
        return deduplicated
    
    def _deduplicate_port_findings(self, findings: List[PortFinding]) -> List[PortFinding]:
        ...
        for finding in findings:
            ...
            if key in unique_findings:
                ...
                if ((finding.service and not existing.service) or
                    (finding.banner and not existing.banner) or
                    (finding.severity.value > existing.severity.value)):
        ...
        for finding in findings:
            ...
            if key in unique_findings:
                ...
                merged_finding = TechnologyFinding(
                    ...
                    # Keep version if it exists in either finding
                    version=finding.version or existing.version,
                    # Keep the categories from the original finding
                    categories=existing.categories,
                    # Use the higher severity
                    severity=(finding.severity if finding.severity.value > existing.severity.value
    ...
    ) -> Dict[str, List[BaseFinding]]:
        """Group related findings by target.
        
        This is a simple correlation approach that groups findings by their target.
        More advanced correlation could be implemented in the future.
        
        Args:
            findings: List of findings to correlate
            
        Returns:
            Dictionary mapping targets to lists of findings
        """
        correlated: Dict[str, List[BaseFinding]] = defaultdict(list)
        
        for finding in findings:
            correlated[finding.target].append(finding)
