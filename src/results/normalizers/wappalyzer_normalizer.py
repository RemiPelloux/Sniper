...
class WappalyzerFindingNormalizer(FindingNormalizer):
    ...
    def __init__(self) -> None:
        ...
        self.high_risk_techs: Set[str] = {
            "wordpress", "joomla", "drupal", "magento",  # Common CMS platforms often targeted
            "apache", "nginx", "iis",  # Web servers
            "php", "asp.net",  # Server-side technologies
    ...
    def normalize(self, raw_findings: List[BaseFinding]) -> List[BaseFinding]:
        ...
        for finding in raw_findings:
            if not isinstance(finding, TechnologyFinding):
                log.warning(f"Non-TechnologyFinding found in Wappalyzer results: {finding}")
                continue
    ...
    def _normalize_description(self, finding: TechnologyFinding) -> str:
        ...
        categories_str = f" (Categories: {', '.join(finding.categories)})" if finding.categories else ""
