...
@patch("src.cli.scan.validate_target_url", return_value="http://localhost:3000")
@patch("src.cli.scan.ScanModeManager")
def test_scan_with_invalid_juiceshop_mode(
    mock_scan_mode_manager_class, mock_validate_target, runner
):
    """Test scan command with mode that doesn't exist"""
    # Setup mock to return None for non-existent mode
    manager_mock = MagicMock()
    manager_mock.get_scan_mode.side_effect = SystemExit(
        "Unknown scan mode: invalid_mode"
    )
    ...
    result = runner.invoke(
        app,
        ["scan", "run", "http://localhost:3000", "--mode", "invalid_mode"],
        catch_exceptions=True,
    )
...
def test_scan_juiceshop_mode_with_unavailable_tools(
    ...
    mock_scan_mode_manager.get_tools_for_scan_mode.return_value = juiceshop_config[
        "tools"
    ]
    ...
    with patch("src.cli.scan.run_technology_scan") as mock_tech_scan, patch(
        ...
        with patch("src.cli.scan.asyncio.run") as mock_asyncio_run:
            ...
            def run_coroutine(coro):
                ...
                console = Console()
                console.print(
                    "Skipping technologies module: Missing required tools: wappalyzer",
                    style="yellow",
                )
                console.print(
                    "Skipping web module: Missing required tools: zap", style="yellow"
                )
                return {}  # Return empty findings
