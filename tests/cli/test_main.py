...
@pytest.mark.skip(reason="Requires refactor of main.py for reliable testing of import-time plugin loading")
...
def test_plugin_loading_success(mock_register, mock_load):
    ...
    mock_register.assert_called_once_with(app) # Ensure it was called with the app instance

@pytest.mark.skip(reason="Requires refactor of main.py for reliable testing of import-time plugin loading")
@patch('src.cli.main.plugin_manager.load_all_plugins', side_effect=Exception("Plugin load failed!"))
@patch('src.cli.main.plugin_manager.register_all_cli_commands')
...
def test_plugin_loading_failure_logged(mock_logger, mock_register, mock_load_fail):
    ...
    assert "Failed during plugin initialization: Plugin load failed!" in mock_logger.call_args[0][0]
