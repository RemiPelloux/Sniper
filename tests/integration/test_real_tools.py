...
def test_list_categories():
    ...
    for category in expected_categories:
        assert category in result.stdout, f"Expected category '{category}' not found in output"
...
def test_list_tools_by_category():
    ...
    category = "reconnaissance"  # Use a category we know exists from the valid categories list
    ...
    assert f"Category: {category}" in result.stdout, f"Category filter '{category}' not mentioned in output"
...
def test_add_and_remove_custom_tool():
    ...
    assert CUSTOM_TOOL_YAML.exists(), f"Custom tool YAML file not found at {CUSTOM_TOOL_YAML}"
    ...
    assert "Successfully added custom tool" in result.stdout, "Success message not found in output"
    ...
    assert show_result.returncode == 0, f"Show tool command failed: {show_result.stderr}"
    assert "test-custom-tool" in show_result.stdout, "Tool name not found in show output"
    assert "A custom test tool created for integration testing" in show_result.stdout, "Tool description not found"
    
    # 5. Now test removing the tool
    remove_result = run_sniper_command(["tools", "remove", "test-custom-tool"])
    assert remove_result.returncode == 0, f"Remove tool command failed: {remove_result.stderr}"
    assert "Removed tool" in remove_result.stdout, "Success message not found in remove output"
    ...
    assert "not found" in final_show.stderr.lower() or "not found" in final_show.stdout.lower(), "Expected 'not found' message after removal"
...
def test_check_updates():
    ...
    assert "Error" not in result.stderr, f"Check updates command had errors: {result.stderr}"
...
def test_install_tool():
    ...
    tool_name = "testssl.sh"  # This is usually a shell script that's relatively lightweight
    ...
    if "Installed" in check_result.stdout:
        pytest.skip(f"Tool {tool_name} is already installed. Skipping installation test.")
    ...
    assert f"Successfully installed {tool_name}" in result.stdout, "Success message not found in output"
    
    # Verify installation via the show command
    show_result = run_sniper_command(["tools", "show", tool_name])
    assert "Installed" in show_result.stdout, "Tool does not show as installed after installation"
...
@pytest.mark.docker
@pytest.mark.skip(reason="Sandbox plugin is not properly configured - 'app/plugins' directory is missing")
def test_sandbox_tool_workflow():
    ...
    try:
        ...
        assert start_result.returncode == 0, f"Failed to start sandbox: {start_result.stderr}"
        
        # 2. Verify sandbox status
        status_result = run_sniper_command(["sandbox", "status", env_name])
        assert status_result.returncode == 0, f"Failed to get sandbox status: {status_result.stderr}"
        ...
        assert list_result.returncode == 0, f"Failed to list sandboxes: {list_result.stderr}"
        assert env_name in list_result.stdout, f"Started sandbox '{env_name}' not found in list"
        
    finally:
        # 4. Stop the sandbox environment, even if tests fail
        stop_result = run_sniper_command(["sandbox", "stop", env_name])
        assert stop_result.returncode == 0, f"Failed to stop sandbox: {stop_result.stderr}"
        
        # 5. Verify it's stopped
        final_status = run_sniper_command(["sandbox", "status", env_name])
        assert "not running" in final_status.stdout.lower() or "stopped" in final_status.stdout.lower(), "Sandbox did not stop properly"
