...
class TestUrlFilter(unittest.TestCase):
    ...
    def test_is_allowed_basic(self):
        ...
        long_url = "http://example.com/page?" + "&".join(
    ...
    def test_is_allowed_excluded_patterns(self):
        ...
        self.assertFalse(
            self.filter.is_allowed("http://example.com/calendar/2023/10/15")
        )

        # URL with UTM parameters should be blocked
        self.assertFalse(
            self.filter.is_allowed("http://example.com/page?utm_source=newsletter")
        )
    ...
    def test_is_allowed_subdomains(self):
        ...
        self.assertTrue(
            filter_with_subdomains.is_allowed("http://blog.example.com/post")
        )
        self.assertTrue(
            filter_with_subdomains.is_allowed("http://store.example.com/product")
        )
        ...
        self.assertFalse(
            filter_without_subdomains.is_allowed("http://blog.example.com/post")
        )
        self.assertFalse(
            filter_without_subdomains.is_allowed("http://store.example.com/product")
        )
    ...
    def test_is_allowed_fragments(self):
        ...
        self.assertTrue(
            self.filter.is_allowed(
                "http://example.com/page1#section", "http://example.com/page2"
            )
        )
    ...
    def test_normalize_url(self):
        ...
        self.assertEqual(
            self.filter.normalize_url("http://example.com"), "http://example.com/"
        )
    ...
    def test_custom_excluded_patterns(self):
        ...
        self.assertFalse(
            custom_filter.is_allowed("http://example.com/restricted/access")
        )
    ...
    def test_handle_invalid_urls(self):
        ...
        self.assertEqual(
            self.filter.normalize_url("javascript:alert(1)"), "javascript:alert(1)"
        )
